...
public class PlaceTable extends Table {
    ...
    public static void getAll(DatabaseHelper db) {
        ITEMS.clear();
    ...
    public static void getPlaceWhereJudul(DatabaseHelper db, String title) {
        ITEMS.clear();
    ...
    public static void getPlaceLike(DatabaseHelper db, String text) {
        ITEMS.clear();
    ...
    public static boolean isEmpty(DatabaseHelper db) {
        return !db.isExist(NAME, null, null, null);
    }

    private static boolean isExist(DatabaseHelper db, String colName, String value) {
        return db.isExist(NAME, new String[]{colName}, colName + "=?", new String[]{value});
    }

    public static boolean isExistPlace(DatabaseHelper db, String title) {
        return isExist(db, COLNAME[0], title);
    }

    public static void add(DatabaseHelper db, RecommendListItem place) {
        db.add(NAME, getValues(place));
    }

    public static void updateWhereJudul(DatabaseHelper db, String title, RecommendListItem place) {
        db.update(NAME, getValues(place), COLNAME[0] + "=?", new String[]{title});
    }

    private static void delete(DatabaseHelper db, String colName, String value) {
        db.delete(NAME, colName + "=?", new String[]{value});
    }

    public static void delete(DatabaseHelper db, RecommendListItem place) {
